...
/* ========== HELPERS ========== */
/* Parse dates from various formats (MySQL datetime, unix timestamps, ISO strings) */
...
function getContactNumber(chatId) {
  if (!chatId) return '';
  const parts = chatId.split('_');
  // chat_id format: {instanceNumber}_{contactNumber}_{uid} or {instanceNumber}_{contactNumber}
  // Take index [1] for contact number
...
function formatPhoneDisplay(num) {
  if (!num) return '';
  // Format: 96871616161 -> +968 7161 6161
  if (num.length === 11 && num.startsWith('968')) {
    ...
    <div className="app">
      ...
      <div className={`sidebar ${isMobile && mobileShowChat ? 'hidden' : ''}`}>
        ...
        <div className="line-tabs">
          {lines.map(line => {
            // Show title name; if name is just a raw number, format it nicer
            let displayName = line.name || line.number;
            if (/^\d+$/.test(displayName) && displayName.length > 8) {
              // Raw number — show last 8 digits formatted
        ...
        <div className={`chat-area ${isMobile && !mobileShowChat ? 'hidden' : ''}`}>
          ...
          <div className="chat-header">
            ...
            <div className="chat-header-info">
              ...
              <div className="chat-header-sub">{formatPhoneDisplay(getContactNumber(activeChat?.chat_id)) || activeChat.sender_mobile || ''}</div>
            </div>
          </div>
